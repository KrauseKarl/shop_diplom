...
urlpatterns = [
    ...
    path(
        "store/<int:pk>/category/<slug:slug>/",
        views.StoreDetailView.as_view(),
        name="category_item",
    ),
    path(
        "store/<int:pk>/sorted/<slug:order_by>/",
        views.StoreDetailView.as_view(),
        name="item_sorted",
    ),
    ...
    path(
        "delivery/detail/<int:pk>/edit/",
        views.DeliveryUpdateView.as_view(),
        name="delivery_edit",
    ),
    path(
        "delivery/order_item/<int:pk>/edit/",
    ...
    path(
        "feature/<slug:slug>/item/<int:pk>/remove/",
        views.RemoveFeatureValueView.as_view(),
        name="value_remove",
    ),
    #
    ...
    path(
        "tag/delete/<int:item_id>/tag/<int:tag_id>/",
        views.RemoveTagFromItem.as_view(),
        name="delete_tag",
    ),
    # path('tag/create/', views.CreateTagView.as_view(), name='create_tag'),
    path(
        "image/delete/<int:pk>/image/", views.DeleteImage.as_view(), name="delete_image"
    ),
    path(
        "image/update/<int:pk>/image/",
        views.MakeImageAsMain.as_view(),
        name="make_image_main",
    ),
    path("export_data_csv/<int:pk>/", views.export_data_to_csv, name="export_data"),
    path(
        "import_data_from_cvs/<int:pk>", views.import_data_from_cvs, name="import_data"
    ),
]
