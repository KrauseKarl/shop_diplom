...
class StoreHandler:
    ...
    def total_profit_store(store) -> int:
        ...
        """
        return (
            OrderItem.objects.filter(item__item__store=store)
            .aggregate(total_profit=Sum("total"))
            .get("total_profit", 0)
        )
    ...
    @staticmethod
    def ordering_store_items(queryset: QuerySet[Item], order_by: str) -> QuerySet[Item]:
        """Функция возвращает queryset товаров отсортированный по заданому знчению сортировки."""
        sort_book = {
