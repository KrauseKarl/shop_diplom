...
class AdminDashBoardView(AdminOnlyMixin, generic.TemplateView):
    """Класс-представление для отображения панели управления администратора."""

    template_name = "app_settings/admin/dashboard.html"

    def get(self, request, *args, **kwargs):
        context = self.get_context_data(**kwargs)
        context["categories"] = item_services.CategoryHandler.admin_category_count()
        context[
            "comments_count"
        ...
        if self.request.GET:
            if self.request.GET.get("search"):
                ...
                object_list = object_list.filter(
                    Q(first_name__startswith=search) | Q(last_name__startswith=search)
                )
...
class CategoryCreateView(AdminOnlyMixin, generic.CreateView):
    ...
    extra_context = {
        "categories": item_models.Category.objects.filter(parent_category=None)
    }
    ...
    def form_valid(self, form):
        ...
        messages.add_message(
            self.request, messages.SUCCESS, f'Категория - "{category_title}" создана'
        )
...
class CategoryUpdateView(AdminOnlyMixin, generic.UpdateView):
    ...
    def form_valid(self, form):
        ...
        category_title = form.cleaned_data.get("title")
        messages.add_message(
            self.request, messages.SUCCESS, f'Категория - "{category_title}" обновлена'
        )
        return redirect("app_settings:category_list")

    def form_invalid(self, form):
        messages.add_message(self.request, messages.ERROR, f"Ошибка обновлений.")
...
class CategoryDeleteView(AdminOnlyMixin, generic.UpdateView):
    ...
    def get(self, request, *args, **kwargs):
        category = item_services.CategoryHandler.get_a_category(kwargs["pk"])
...
class TagUpdateView(AdminOnlyMixin, generic.UpdateView):
    ...
    def form_valid(self, form):
        ...
        messages.add_message(
            self.request, messages.SUCCESS, f'Тег - "{tag_title}" изменен'
        )
        return redirect("app_settings:tag_list")
...
class TagDeleteView(AdminOnlyMixin, generic.UpdateView):
    ...
    def get(self, request, *args, **kwargs):
        tag_id = kwargs["pk"]
        try:
            tag = item_services.TagHandler.get_a_tag(tag_id=tag_id)
            if tag.is_active:
                tag.is_active = False
...
class FeatureListView(AdminOnlyMixin, generic.DetailView):
    ...
    def get(self, request: HttpRequest, *args, **kwargs) -> HttpResponse:
        ...
        values = item_models.FeatureValue.all_objects.select_related("feature").filter(
...
class FeatureCreateView(AdminOnlyMixin, generic.CreateView):
    ...
    def form_valid(self, form):
        ...
        category = item_services.CategoryHandler.get_a_category(category_id=category_id)
        feature.categories.add(category.id)
        messages.add_message(
            self.request, messages.SUCCESS, f'Характеристика - "{feature}" добавлено'
        )
    ...
    def form_invalid(self, form):
        ...
        messages.add_message(
            self.request, messages.ERROR, f"Произошла ошибка - {form.errors}"
        )
        category_id = self.kwargs.get("pk")
...
class FeatureDeleteView(AdminOnlyMixin, generic.UpdateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            feature = item_services.FeatureHandler.get_all_features(
...
class ValueCreateView(AdminOnlyMixin, generic.CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.add_message(
            self.request, messages.SUCCESS, f'Значение - "{value}" добавлено'
        )
...
class CommentListView(AdminOnlyMixin, generic.ListView, MixinPaginator):
    ...
    def get(self, request, *args, **kwargs):
        object_list = item_services.CommentHandler.get_all_comments()
        if request.GET:
